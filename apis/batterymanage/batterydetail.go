...
// @Security Bearer
func GetBatteryDetail_dtu_statusinfo(c *gin.Context) {
...
//2006-01-02 15:04:05.9999999 +0800
	if date := c.Request.FormValue("startdate"); date != "" {
	...
	if date := c.Request.FormValue("enddate"); date != "" {
		l,err := time.LoadLocation("Local")
		if err != nil {
			fmt.Println(err)
		}
	...
	result,count, err := data.Getdtu_statusinfo(startdate, enddate)
	fmt.Println(count)
